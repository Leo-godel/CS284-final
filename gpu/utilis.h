...
__device__ inline double bspline(double x) {
	x = fabs(x);
	double w;
	if (x < 1)
		w = x * x * (x / 2 - 1) + 2 / 3.0;
...
__device__ inline double bsplineSlope(double x) {
...
__device__ inline Matrix2D outer_product(Vector2D& a, Vector2D& b) {
	return Matrix2D(a.x * b.x, a.x * b.y, a.y * b.x, a.y * b.y);
}

__device__ inline void polarDecomp(Matrix2D m, Matrix2D& R, Matrix2D& S) {
	auto x = m(0, 0) + m(1, 1);
	auto y = m(1, 0) - m(0, 1);
...
__device__ inline void svd_0(Matrix2D m, Matrix2D& U, Matrix2D& sig, Matrix2D& V) {
	Matrix2D S;
	polarDecomp(m, U, S);
	double c, s;
	if (fabs(S(0, 1)) < 1e-6) {
...
__device__ inline void svd(Matrix2D m, Matrix2D& U, Matrix2D& sig, Matrix2D& V){
